...
public class ActionSubscriber 
{
	...
	public static void main(String[] args) throws Exception 
	{
		...
		Hashtable actions = (Hashtable) ictx.lookup("liste_actions");
		Set<String> keys = actions.keySet();
		String[] liste_actions = keys.toArray(new String[keys.size()]);
		for(int i=0; i<liste_actions.length; i++)
			System.out.println(i + " - " + liste_actions[i] + " parent : "+actions.get(liste_actions[i]));
